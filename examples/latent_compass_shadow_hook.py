"""Host-side hook wrapper for passive Latent Compass shadow advice.

This script owns the process boundary that the package deliberately forbids.
It adds a bounded Git source declaration on lifecycle events, then hands the
sanitised envelope to :mod:`latent_compass.shadow_harness`.
"""
    ...
    *,
    ...
    try:
        ...
        config = load_shadow_config(
            json.loads((store / DEFAULT_CONFIG_NAME).read_text(encoding="utf-8"))
        )
        ...
        if event_name in {"SessionStart", "PostToolUse"}:
            ...
            if root is not None:
                ...
                payload["source_observed_at"] = args.now
                if args.now is None:
