...
child or an orphan behind, fail, write on stderr. No real agent session runs here and no
key exists. These tests prove nothing about the experiment, which has not run.
"""
...
class Rig:
    ...
    def command_line(self) -> list[str]:
        ...
        tasks.write_text(
            json.dumps([{"task_id": "throwaway-1", "prompt": "look"}]), encoding="utf-8"
        )
        arguments = ["--key-file", str(self.key_file), "--source", str(self.source)]
        arguments += ["--ref", "HEAD", "--tasks", str(tasks), "--out", str(self.out)]
    ...
    def isolated_home(self) -> Path:
        return Path(self.calls("login")[0]["codex_home"])
...
def test_the_allowance_of_six_sessions_belongs_to_the_key_not_to_the_output_directory(
    ...
    second = rig.run(["five", "six", "seven", "eight"], out=rig.out.with_name("another-out"))
    assert second["sessions_launched"] == 6
    assert [record["task_id"] for record in second["records"]] == ["throwaway-1", "throwaway-2"]
...
def test_the_report_holds_durations_and_usage_and_no_text_from_the_agent(rig: Rig) -> None:
    ...
    done, failed = report["records"]
    assert (done["status"], failed["status"]) == ("COMPLETED", "TURN_FAILED")
    assert report["tokens"]["input_tokens"]["max"] == 1000
...
def test_a_directory_is_reported_removed_only_when_it_is_gone(
    ...
    # A removal that silently fails is not reported as a success...
    ...
    # ...and what could be removed was: the file that would hold the key is gone.
    assert not (stuck / "deep" / "auth.json").exists()
...
def test_the_cli_is_never_resolved_from_the_current_directory(
    ...
    monkeypatch.setenv("PATH", os.pathsep.join([str(rig.source), ".", ""]))
...
def test_the_agent_gets_an_isolated_home_and_none_of_the_parent_environment(
    ...
    parent_only.mkdir()
    monkeypatch.setenv("PATH", os.environ["PATH"] + os.pathsep + str(parent_only))
    monkeypatch.setenv("OPENAI_API_KEY", "parent-secret")
    monkeypatch.setenv("CODEX_HOME", str(rig.source))
    rig.run(["look around"])
    calls = rig.calls()
    assert [call["argv"][0] for call in calls] == ["login", "--version", "exec"]
    for call in calls:
...
def test_a_session_that_overruns_is_killed_with_its_children(
    ...
    # The grandchild left a mark every tenth of a second while it lived. It has to have stopped.
    time.sleep(0.5)
    first = rig.heartbeats()
    assert first > 0
    time.sleep(0.7)
    assert rig.heartbeats() == first
...
class _Interrupted(subprocess.Popen[bytes]):
    """A Popen whose first wait on an agent session is interrupted, as by Ctrl+C."""
...
def test_an_interruption_kills_the_paid_session_before_it_propagates(
    rig: Rig, monkeypatch: pytest.MonkeyPatch
) -> None:
